...
interface InsumoDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  editingIng: Ingrediente | null;
  ingredientes: Ingrediente[];
  onRefresh: () => void;
}

export const InsumoDrawer: React.FC<InsumoDrawerProps> = ({
  isOpen,
  onClose,
  editingIng,
  ingredientes,
  ...
  // Sincronizar form cuando se edita desde afuera
  useEffect(() => {
    if (editingIng) {
      setIngForm({
      ...
      <div className={`drawer-panel ${isOpen ? 'open' : ''}`}>
        <form onSubmit={handleSaveIngrediente} style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          <div className="drawer-header">
            <h2>{activeIng ? 'Modificar Insumo' : 'Nuevo Insumo'}</h2>
            <button 
              type="button" 
              ...
              <div className="ref-insumos-container" style={{
                ...
                {(() => {
                  ...
                  const filtered = ingredientes.filter(ing => {
                    const conservacionText = ing.conservacion === 'secos' ? 'secos almacén despensa' : ing.conservacion === 'refrigerado' ? 'refrigerado refrigeración' : 'congelado congelación';
                    return (
          ...
          <div className="drawer-footer">
            ...
            >
              {activeIng ? 'Guardar Cambios' : 'Añadir Insumo'}
            </button>
          </div>
        </form>
      </div>
